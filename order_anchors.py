"""
Code for traveling salesman problem solution copied from:
https://github.com/dmishin/tsp-solver

Credit to Dmitry Shintyakov for TSP code (starting after block comment)
"""
...
class Anchor:
    def __init__(self, coords):
        """
        Takes the input (x-coord, y-coord) tuple. Creates attributes for x and y coord.
        """
    ...
    def dist_to(self, other_anchor):
        """
        Takes another anchor as input. Returns the distance between the two anchors.
        """
        
        x_diff = self.x - other_anchor.x
        y_diff = self.y - other_anchor.y
        dist = (x_diff**2 + y_diff**2)**0.5
        return dist
    
    def all_dists(self, list_of_anchors):
        """
        Takes a list of anchors as input. Returns a list holding the distance from 
        """
    
    def nearest_neighbor(self, list_of_anchors):
        """
        Takes a list of anchors as input. Returns the anchor closest to self.
        """
        
        nearest = min([self.dist(anchor) for anchor in list_of_anchors])
        return nearest
...
class Graph:
    def __init__(self, anchor_list):
        """
        Takes a list of all anchors (each as a tuple) as input. 
        Creates attributes for a list of all anchors, and a list of anchors yet to be ordered.
        """
        
        self.all_anchors = [Anchor(item) for item in anchor_list]
        
    def make_dist_matrix(self):
        """
        Returns a matrix of the distances between each two anchors.
        """
        
        D = np.zeros([len(self.all_anchors), len(self.all_anchors)])
        for i in range(len(self.all_anchors)):
            for j in range(len(self.all_anchors)):
                anchor1 = self.all_anchors[i]
                anchor2 = self.all_anchors[j]
                D[i][j] = anchor1.dist_to(anchor2)
        return(D)
    
    def get_shortest_path(self, D):
        """
        Given a distance matrix, returns the shortest path through the anchors.
        """
        
        path = solve_tsp(D)
        return (path)
    
    def order_anchors(self, path):
        """
        Given a path denoted by indices, returns a list of all anchors sorted in the order of the path.
        """
        
        ordered_anchors = [self.all_anchors[i].coords for i in path]
        return ordered_anchors

def order_anchors(anchor_list):
    G = Graph(anchor_list)
    D = G.make_dist_matrix()
    path = G.get_shortest_path(D)
    ordered_anchors = G.order_anchors(path)
    return (ordered_anchors)
